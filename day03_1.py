...
자료형(type) : 데이터의 타입
    - str, int, float
    - 오류검출, 실수검출, 의도명확화
    - 뺄셈으로 사용하고자하는지, 핸드폰 번호로 사용하고자 하는지 컴퓨터는 알 수 없음
    - 파이썬에서 제공해주는 기능(함수)를 잘 못 사용했을 때 오류로 검출
    - str : 문자열, int : 정수, float : 실수(소수점 있는 숫자)
...
while 조건식:
    조건식이 True 일때만 실행할 문장1
    조건식이 True 일때만 실행할 문장2
반복문은 실행되면 위로 다시 올라감 (while부분으로 올라가서 조건식 검사부터 다시함)
'''
